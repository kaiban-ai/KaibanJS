...
import { ENUM_TASK_STATUS } from "../utils/enums";
        ...
        if (newLog.logType === "TaskStatusUpdate") {
          ...
          switch (newLog.task.status) {
            case ENUM_TASK_STATUS.DONE:
              ...
              break;
            case ENUM_TASK_STATUS.DOING:
            case ENUM_TASK_STATUS.BLOCKED:
            case ENUM_TASK_STATUS.REVISE:
            case ENUM_TASK_STATUS.TODO:
              logPrettyTaskStatus({
                currentTaskNumber,
                totalTasks,
                taskTitle: getTaskTitleForLogs(newLog.task),
