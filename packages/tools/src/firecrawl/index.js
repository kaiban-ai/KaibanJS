...
export class Firecrawl extends Tool {
    constructor(fields) {
        ...
        this.mode = 'scrape';
        this.description =
            `Fetches web content from a specified URL and returns it in ${this.format} format. Input should be a JSON object with a "url".`;
    ...
    async _call(input) {
        try {
            const response = await this.httpClient.post('https://api.firecrawl.dev/v1/scrape', {
                json: {
                    url: input.url,
                    formats: [this.format],
                },
            ...
            return response?.data?.[this.format] || 'The API returned an empty response.';
        } catch (error) {
