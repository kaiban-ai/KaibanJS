...
function generateUUID(): string {
  // Try to use crypto.randomUUID() (available in Node.js 14.17+ and modern browsers)
  if (typeof crypto !== 'undefined' && crypto.randomUUID) {
    return crypto.randomUUID();
  }

  // Fallback for older environments - simple UUID v4 implementation
...
> implements Step<TInput, TOutput>
{
  ...
  createRun(options?: { runId?: string }): Run<TInput, TOutput> {
    ...
    const runIdToUse = options?.runId || generateUUID();
