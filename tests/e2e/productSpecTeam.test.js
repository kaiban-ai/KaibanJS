        ...
        it('(1) - handles task requiring validation', (done) => {
            ...
            team.onWorkflowStatusChange((status) => {
                if (status === 'FINISHED') {
                    ...
                    expect(finalState).toMatchSnapshot('State after task validation and workflow completion');
                    done(); // Signal that the test is complete
                } else if (status === 'BLOCKED') {
                    // Check the state when the workflow is blocked
                    const blockedState = team.useStore().getState().getCleanedState();
                    // eslint-disable-next-line jest/no-conditional-expect
                    expect(blockedState).toMatchSnapshot('State when workflow is blocked waiting for validation');
        ...
        it('(2) - processes feedback and completes workflow', (done) => {
            ...
            team.onWorkflowStatusChange((status) => {
                if (status === 'FINISHED') {
                    const finalState = team.useStore().getState().getCleanedState();
                    // eslint-disable-next-line jest/no-conditional-expect
                    expect(finalState).toMatchSnapshot('State after feedback, validation, and workflow completion');
                    done(); // Signal that the test is complete
                } else if (status === 'BLOCKED') {
                    const blockedState = team.useStore().getState().getCleanedState();
                    ...
                    if (!feedbackProvided && tasksAwaitingValidation.length > 0) {
                        ...
                        // eslint-disable-next-line jest/no-conditional-expect
                        expect(blockedState).toMatchSnapshot('State when workflow is blocked waiting for initial feedback');
                        const taskNeedingFeedback = tasksAwaitingValidation[0];
                        team.provideFeedback(taskNeedingFeedback.id, "Sorry the founder idea is to spent 10k in Google Ads every");
                        feedbackProvided = true;
                    } else if (feedbackProvided && tasksAwaitingValidation.length > 0) {
                        // Second time blocked: validate the task
                        // eslint-disable-next-line jest/no-conditional-expect
                        expect(blockedState).toMatchSnapshot('State when workflow is blocked after feedback, waiting for validation');
