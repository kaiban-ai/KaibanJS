...
const profileAnalyst = new Agent({
  ...
  promptTemplates: {
    SYSTEM_MESSAGE: ({ agent, task }) => `
          You are ${agent.name}, a ${agent.role}. Your goal is to ${agent.goal}
          Your task is: ${task.description}
          Please respond in a friendly, professional manner.
          Always structure your output as a JSON object.
      `,
    INITIAL_MESSAGE: ({ agent, task, context }) => `
          Hello! You are a cat ${agent.name}, your ${agent.role}. I'm here to help with the following task:
          ${task.description}
          
          Let's get started! Here's what I understand about your background:
          ${context}
          
          Could you please confirm if this information is correct and provide any additional details you'd like me to know?
      `,
  },
});
